		...
		<div class="reply_panel panel-body">
			<div class="reply_tit panel-heading">
				...
				<button id="addReplyBtn"
					class="reply_btn btn-primary btn-xs pull-right">New Reply</button>
			</div>
			<ul class="chat">
				<!-- start reply -->
...
<script type="text/javascript">
	...
	function showList(page) {
		...
		reply.getList({bno:bnoValue,page: page||1}, function (replyCnt, list) {
			...
			for (var i = 0, len = list.length || 0; i < len; i++) {
				...
				str += "<small class='pull-right text-muted'>"
						+ reply.displayTime(list[i].replyDate)
						+ "</small></div>";
	...
	replyPageFooter.on("click", "li a", function (e) {
		...
		//모달 (댓글)등록 및 갱신 'Reply' 'Replyer' 이벤트
 		modalRegisterBtn.on("click", function(e) {
			...
			reply.add(reply, function(result) {
				alert(result);
		...
		//모달(댓글)조회 Ajax처리 수정과 삭제가 필요한 댓글 번호(rno) 'data-rno' 속성 만들어서 추가
 		$(".chat").on("click","li", function (e) {
 			...
 			reply.get(rno, function (reply) {
 				modalInputReply.val(reply.reply);
 				modalInputReplyer.val(reply.replyer);
 				modalInputReplyDate.val(reply.displayTime(reply.replyDate)).attr("readonly","readonly");
		...
		//모달(댓글) 수정, 페이징 목록
 		modalModBtn.on("click", function (e) {
 			...
 			reply.update(reply, function (result) {
 				alert(result);
 				modal.modal("hide");
		...
		//모달(댓글) 삭제
 		modalRemoveBtn.on("click", function (e) {
 			...
 			reply.remove(rno, function (result) {
 				alert(result);
 				modal.modal("hide");
